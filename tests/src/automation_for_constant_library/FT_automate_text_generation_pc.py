...
def download_and_process_dataset():
    ...
    exit_status = os.system("python ./download_datasetn.py --dataset_subset generation --dataset truthful_qa --download_dir truthful_qa-dataset")
    if exit_status != 0:
    ...
    frac = 0.1
    train_df.sample(frac=frac).to_json("./truthful_qa-dataset/small_train.jsonl", orient="records", lines=True)
    validation_df.sample(frac=frac).to_json("./truthful_qa-dataset/small_validation.jsonl", orient="records", lines=True)
    test_df.sample(frac=frac).to_json("./truthful_qa-dataset/small_test.jsonl", orient="records", lines=True)
