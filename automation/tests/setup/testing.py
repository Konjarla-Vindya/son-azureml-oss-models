...
def create_workflow_files(queue, workspace_list, df):
    counter=0
    ...
    for workspace in queue:
        for thread in queue[workspace]:
            for model in queue[workspace][thread]:
                model_name = df[df['modelName'] == model]['modelId']
                model_name = re.sub(pattern, '', model_name.to_string())
                write_single_workflow_file(model, model_name, f"{workspace}-{thread}", workspace_list[workspace]['secret_name'])
                # print progress
                counter=counter+1
...
def write_single_workflow_file(model, model_name, q, secret_name):
    ...
    os.system(f"sed -i 's/<test_model_name>/{model_name}/g' {workflow_file}")
...
def main():
    ...
    fields = ['modelId']
    df = pd.read_csv(filepath_or_buffer="Exception_task_models.csv", usecols=fields)
    df["modelName"] = df.modelId.apply(lambda x: x.replace("/", "-"))
    #df[model_name]
    print(df.head(10))
    df = df.iloc[0:2]
    models = df['modelName'].to_list()
    ...
    create_workflow_files(queue, workspace_list, df)
